    ...
    PUNICODE_STRING RemainingName, MY_CONTEXT *MyContext, BOOLEAN msg1313)
{
    ...
    if (! IsBoxedPath) {
        ...
        //
        // if process is trying to load a special DLL before SbDll
        // has been initialized, then pretend the file does not exist,
        // and add the DLL path so it can be loaded by SbDll
        //

        if ((! proc->sbdll_loaded) && status == STATUS_SUCCESS
...
skip_due_to_home_folder:
        ...
        if (! IsPipeDevice) {
            ...
            // a call to NtFsControlFile.  instead, SbDll will use the
...
_FX NTSTATUS File_Api_Open(PROCESS *proc, ULONG64 *parms)
{
    ...
    // by File_NtQueryFullAttributesFile in SbDll
    //
