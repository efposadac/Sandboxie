...
LdrLoadDll_Good:
		;
		; call LdrLoadDll for sbdll
		;

		lea	eax, [esi].InjectData.ModuleHandle
		push	eax
		lea	eax, [esi].InjectData.SbDll_Unicode
		push	eax
		push	0
		push	0
		call	dword ptr [esi].InjectData.LdrLoadDll
		...
		;
		; call LdrGetProcedureAddress for sbdll ordinal 1,
		; which forces ntdll to initialize sbdll
		;
		
		lea	eax, [esi].InjectData.SbDllOrdinal1
		push	eax
		push	1
		push	0
		...
		mov	eax, esi
		pop	esi
		jmp	dword ptr [eax].InjectData.SbDllOrdinal1
...
RtlFindActivationContextSectionStringError:
    	...
    	lea	edx, [esi].InjectData.SbDll_Unicode
...
LdrLoadGood:
		...
		xor	rcx, rcx
		xor	rdx, rdx
		lea	r8, [rsi].InjectData.SbDll_Unicode
		lea	r9, [rsi].InjectData.ModuleHandle
		call	qword ptr [rsi].InjectData.LdrLoadDll
		...
		xor	r8, r8
		inc	r8
		lea	r9, [rsi].InjectData.SbDllOrdinal1
		...
		add	rsp, 8*8
		pop	rsi
		jmp	qword ptr [rcx].InjectData.SbDllOrdinal1
...
RtlFindActivationContextSectionStringError:
		...
		mov	r8, rdx		; mask_of_strings_in_list
    		...
    		lea	rax, [rsi].InjectData.SbDll_Unicode
