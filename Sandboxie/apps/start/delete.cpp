...
void Error(const WCHAR *Descr, NTSTATUS Status)
{
    ...
    if (! g_Silent) {
        ...
        WCHAR text[512];
        wcscpy(text, SbDll_FormatMessage1(MSG_3214, g_BoxName));
        wcscat(text, Descr);
        if (Status) {
    ...
    const WCHAR *BoxName, BOOL bLogoff, BOOL bSilent, int phase)
{
    ...
    if (SbieApi_QueryProcess(NULL, NULL, NULL, NULL, NULL) == 0) {
        SetLastError(0);
        Error(SbDll_FormatMessage0(MSG_3221), 0);
    }
    ...
    if ((phase <= 1) &&
        ...
        SetLastError(0);
        Error(SbDll_FormatMessage0(MSG_3051), 0);
...
void RenameSandbox(void)
{
    ...
    if (! boxpath)
        Error(SbDll_FormatMessage0(MSG_3216), 0);
    ...
    if (status != STATUS_SUCCESS) {
        ...
        Error(SbDll_FormatMessage0(MSG_3217), status);
    }
    ...
    *backslash = L'\0';
    ...
    if (status != STATUS_SUCCESS)
        Error(SbDll_FormatMessage0(MSG_3218), status);
    ...
    while (1) {
        ...
        if (retries == 30)
            Error(SbDll_FormatMessage0(MSG_3219), status);
...
void LaunchProgram(WCHAR *cmdSrc, bool bWait)
{
    ...
    if (! ok) {
        WCHAR txt[1024];
        wcscpy(txt, SbDll_FormatMessage0(MSG_3222));
        wcscat(txt, cmd);
        Error(txt, 0);
    }
}
    ...
    while (1) {
        ...
        while (1) {
            ...
            if ((! needRename) && (name_len <= 8)) {
                if(SbDll_IsReservedFileName(name))
                    needRename = true;
            }
            ...
            if (needRename) {
                ...
                if (! ok) {
                    Error(SbDll_FormatMessage0(MSG_3220), 0);
                    return;
                }
                anyRenames = true;
...
ALIGNED WCHAR *GetBoxFilePath(const WCHAR *boxname, ULONG extra)
{
    ...
    if (path) {
        ...
        if (status == 0) {
            BOOLEAN ok = SbDll_TranslateNtToDosPath(path);
            if (! ok)
                status = 1;
        }
